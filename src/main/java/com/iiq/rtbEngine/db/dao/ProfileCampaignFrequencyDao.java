...
public class ProfileCampaignFrequencyDao {
    ...
    private static final String INSERT_STATEMENT = "INSERT INTO " + PROFILE_CAMPAIGN_FREQUENCY_TABLE_NAME +
            " VALUES (%d, %d, %d) " +
            "ON CONFLICT (" + PROFILE_ID_COLUMN + ", " + CAMPAIGN_ID_COLUMN + ") " +
            "DO UPDATE SET " + FREQUENCY_COLUMN + " = " + FREQUENCY_COLUMN + " + EXCLUDED." + FREQUENCY_COLUMN;
    ...
    public void insertFrequency(int profileId, int campaignId, int frequency) {
        try {
            ...
            logger.error("Error while trying to insert frequency into table " + PROFILE_CAMPAIGN_FREQUENCY_TABLE_NAME, e);
        }
    }
    ...
    public Integer getFrequency(int profileId, int campaignId) {
        try {
            ...
            logger.error("Error while trying to retrieve frequency from table " + PROFILE_CAMPAIGN_FREQUENCY_TABLE_NAME, e);
        }
        return null;
    }
}
