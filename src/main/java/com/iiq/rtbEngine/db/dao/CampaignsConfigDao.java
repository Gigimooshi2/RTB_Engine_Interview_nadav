...
public class CampaignsConfigDao {
	@Autowired
	private H2DB h2Db;
	private static final Log logger = LogFactory.getLog(CampaignsConfigDao.class);
	...
	public void createTable() {
		try {
			...
			logger.error("Error while trying to create table "+CAMPAIGN_CONFIG_TABLE_NAME,e);
		}
	}
	...
	public void updateTable(String campaignId, String capacity, String priority) {
		try {
			...
			logger.error("Error while trying to update table "+CAMPAIGN_CONFIG_TABLE_NAME+" with campaignId="+campaignId+" capacity"+capacity+" priority="+priority,e);
		}
	}
	...
	public CampaignConfig getCampaignConfig(int campaignId) {
		try {
			...
			logger.error("Error while trying to retrieve campaign configurations from DB for campaign "+campaignId,e);
		}
		return null;
	}
	...
	public Map<Integer, CampaignConfig> getAllCampaignsConfigs() {
		...
		try {
			...
			logger.error("Error while trying to retrieve all campaigns configurations from DB",e);
		}
